...
namespace service {
	...
	export function pullNotify(eventHandler) {
		function pull() {
			ajax.get("notify", { session: session }, function (err, result) {
				if (pullStoped)
					return;
				if (err) {
					console.error(err);
	...
	export function pollNotify(url: string, cb: Function) {
		let params = { session: service.session };
		function poll() {
    ...
    export function filterItem(item: { id: number, value: string }, value: string) {
